...
let pubkey = secp256k1.createPublicKey(privateKey);
let hash_256 = SHA256(pubkey);
let hash_160 = RIPEMD160(pubkey);
*/
...
class UnspentTxOut {
    ...
    constructor(txOutId: string, txOutIndex: number, address: string, amount: number) {
        ...
        this.amount = amount;
        this.hash = CryptoJS.RIPEMD160(CryptoJS.SHA256(address));
    }
}
...
const validateTxIn = (txIn: TxIn, transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]): boolean => {
    ...
    const hash = referencedUTxOut.hash;

    const key = ec.keyFromPublic(hash, 'hex');
