...
const createTransaction = (receiverAddress: string, amount: number, privateKey: string,
    ...
    /////
    const HASH160:string = CryptoJS.RIPEMD160(CryptoJS.SHA256(PUBKey));
    /////
    
    const myUnspentTxOutsA = unspentTxOuts.filter((uTxO: UnspentTxOut) => uTxO.hash === HASH160);/////*****/////
    ...
    tx.txOuts = createTxOuts(receiverAddress, HASH160, amount, leftOverAmount);
...
function LOCK_P2PKH(PUBKey, signture) {
  return (CryptoJS.RIPEMD160(CryptoJS.SHA256(PUBKey)) == HASH160) && CheckSig(pubKey, signature)
}
